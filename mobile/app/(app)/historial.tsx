...
export default function HistorialScreen() {
  ...
  const entregadas = (manifiesto.data?.paradas ?? []).filter((p) => p.estado_entrega === "ENTREGADO");
    ...
    <Screen conPadding={false}>
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return (
            <Card>
              <Text style={estilos.codigo}>{item.codigo ?? `Pedido ${item.pedido_id}`}</Text>
              <Text style={estilos.destinatario}>{item.nombre_destinatario || item.cliente_origen}</Text>
              <Text style={estilos.direccion}>{item.direccion_destino}</Text>
              {evidencia ? (
                <Image source={{ uri: evidencia }} style={estilos.evidencia} contentFit="cover" />
              ) : (
                <Text style={estilos.sinEvidencia}>Entregado (evidencia no disponible en esta sesión)</Text>
              )}
            </Card>
          );
        }}
...
const estilos = StyleSheet.create({
  ...
  codigo: { fontSize: fontSize.subtitle, fontWeight: "800", color: colors.ink },
  destinatario: { fontSize: fontSize.body, color: colors.text, marginTop: 2, fontWeight: "600" },
  direccion: { fontSize: fontSize.body, color: colors.muted, marginTop: 2 },
  evidencia: { width: "100%", height: 180, borderRadius: radius.md, marginTop: spacing.md },
  sinEvidencia: { color: colors.success, marginTop: spacing.sm, fontSize: fontSize.caption, fontWeight: "600" },
});
